...
1. Запустить контейнер с базой и веб-роутером (в корне проекта):
   ...
   Веб-роутер на 80 порту будет перенаправлять запросы с `*.orbitar.test` на `localhost:5000` (фронт), а `api.orbitar.test` на `localhost:5001` (бэк).
...
После первого запуска можно открыть приглашение http://orbitar.test/invite/initial и зарегистрировать первый юзернейм.
