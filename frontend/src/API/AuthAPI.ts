...
export default class AuthAPI {
  ...
  async status(): Promise<[StatusResponse, string]> {
    ...
    const status = await this.api.request<StatusRequest, StatusResponse>('/status', {}, (resp) => {
  ...
  async dropPasswordAndSessions() {
    return await this.api.request<ResetPasswordAndSessionsRequest, ResetPasswordAndSessionsResponse>(
      '/auth/drop-password-and-sessions',
      {},
    )
  }
  ...
  async checkResetPasswordCode(code: string): Promise<ResetPasswordResponse> {
    return this.api.request<CheckResetPasswordCodeRequest, ResetPasswordResponse>('/auth/check-reset-password-code', {
      code,
    })
  }
}
