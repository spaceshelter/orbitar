...
export default function PostPage() {
  ...
  const [scrolledToComment, setScrolledToComment] = useState<{
    postId: number;
    commentId: number;
  }>();
  ...
  const { post, comments, postComment, editComment, editPost, error, reload, updatePost } = usePost(
    site,
    postId,
    unreadOnly,
  );
  ...
  useEffect(() => {
    ...
    if (location.hash) {
      ...
      if (
        scrolledToComment &&
        scrolledToComment.postId === postId &&
        scrolledToComment.commentId === commentId
      ) {
        return;
      }
