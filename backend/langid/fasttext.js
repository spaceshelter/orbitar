...
class FastTextModel {
  ...
  predict(text, k = 1, threshold = 0.0) {
      const labels = this.f.predict(text, k, threshold);
      const res = [];
      for (let i = 0; i < labels.size() && i < k; i++) {
          const label = labels.get(i);
          label[1] = label[1].replace('__label__', '');
          res.push(label);
      }
      return res;
  }
...
module.exports = { FastText, FastTextModel, addOnPostRun };