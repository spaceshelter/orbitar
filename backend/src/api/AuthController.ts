...
export default class AuthController {
  ...
  constructor(userManager: UserManager, logger: Logger) {
    ...
    this.router.post('/auth/reset-password', resetPaswsordLimiter, validate(resetPasswordSchema), (req, res) =>
      this.resetPaswsord(req, res),
    )
    this.router.post('/auth/drop-password-and-sessions', (req, res) => this.dropPasswordAndSessions(req, res))
    this.router.post('/auth/new-password', resetPaswsordLimiter, validate(newPasswordSchema), (req, res) =>
      this.setNewPassword(req, res),
    )
    this.router.post('/auth/check-reset-password-code', validate(checkResetPasswordCodeSchema), (req, res) =>
      this.checkIfPasswordResetCodeExists(req, res),
    )
  ...
  async checkIfPasswordResetCodeExists(
    request: APIRequest<AuthCheckResetPasswordCodeRequest>,
    response: APIResponse<AuthCheckResetPasswordCodeResponse>,
  ) {
  ...
  async dropPasswordAndSessions(
    request: APIRequest<DropPasswordAndSessionsRequest>,
