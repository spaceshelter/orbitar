...
export class ExpressOauth2ScopesFilter {
    ...
    getAllScopes() {
        if (this.allScopes === undefined) {
            this.allScopes = Array.from(new Set(Object.keys(this.getPaths()).reduce((acc, path) => {
    ...
    static isSubscope(subscope: string, superscope: string) {
        return subscope === superscope || (
            subscope.length > superscope.length &&
            subscope[superscope.length] === ':' &&
            subscope.startsWith(superscope)
        );
    }
    ...
    static minimizeScopes(arr: string[]) {
        return arr.filter(scope =>
            scope.indexOf(':') === -1 ||
            !arr.some(superscope => ExpressOauth2ScopesFilter.isSubscope(superscope, scope))
        );
    }
    ...
    static pathToScopesList(path: string) {
        ...
        //     'oauth2:client:register'
        //   ]
        return path.split('/').slice(1).reduce((acc, chunk) => {
            chunk = chunk.trim();
