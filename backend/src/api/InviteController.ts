...
export default class InviteController {
  ...
  constructor(
    ...
    this.router.post('/invite/check', limiter, validate(checkSchema), oauth('проверка инвайта'), (req, res) =>
      this.checkInvite(req, res),
    )
    this.router.post('/invite/use', limiter, validate(useSchema), oauth('использование инвайта'), (req, res) =>
      this.useInvite(req, res),
    )
    this.router.post('/invite/list', limiter, validate(listSchema), oauth('список инвайтов'), (req, res) =>
      this.list(req, res),
    )
    this.router.post('/invite/regenerate', limiter, validate(codeSchema), oauth('перегенерация инвайта'), (req, res) =>
      this.regenerate(req, res),
    )
    this.router.post('/invite/create', limiter, validate(createSchema), oauth('создание инвайта'), (req, res) =>
      this.create(req, res),
    )
    this.router.post('/invite/delete', limiter, validate(codeSchema), oauth('удаление инвайта'), (req, res) =>
      this.delete(req, res),
    )
    this.router.post('/invite/edit', limiter, validate(editSchema), oauth('редактировать инвайты'), (req, res) =>
      this.edit(req, res),
    )
  }
  ...
  async verifyInvitePermissions(invite: InviteRawWithIssuer) {
    ...
    if (!issuerRestrictions.canInvite) {
      this.logger.warn(`Invite issuer ${invite.issued_by} has no permission to invite`, { invite: invite.code })
      return false
    }
    return true
  }
  ...
  async list(request: APIRequest<InviteListRequest>, response: APIResponse<InviteListResponse>) {
    ...
    try {
      ...
      const invitesAvailability =
        isSelf && canInvite && (await this.inviteManager.getInvitesAvailability(forUser.id, true))
