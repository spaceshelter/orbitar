...
export default class InviteController {
    ...
    async list(request: APIRequest<InviteListRequest>, response: APIResponse<InviteListResponse>) {
        ...
        try {
            ...
            const invitesAvailability = isSelf && canInvite && await this.inviteManager.getInvitesAvailability(forUser.id);
    ...
    async create(request: APIRequest<InviteCreateRequest>, response: APIResponse<InviteEntity>) {
        ...
        try {
            ...
            if ((await this.inviteManager.getInvitesAvailability(userId)).invitesLeft === 0) {
