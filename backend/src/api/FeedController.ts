...
export default class FeedController {
  ...
  constructor(
    ...
    this.router.post('/feed/subscriptions', validate(feedSubscriptionsSchema), (req, res) =>
  ...
  async feedSubscriptions(
    request: APIRequest<FeedSubscriptionsRequest>,
    response: APIResponse<FeedSubscriptionsResponse>,
  ) {
    ...
    try {
      ...
      const { total, posts: rawPosts } = restrictedPosts
        ? {
  ...
  async feedAll(request: APIRequest<FeedSubscriptionsRequest>, response: APIResponse<FeedSubscriptionsResponse>) {
    ...
    try {
      ...
      const total = restrictedPosts
        ...
        : await this.feedManager.getAllPostsTotal()
  ...
  async feedPosts(request: APIRequest<FeedPostsRequest>, response: APIResponse<FeedPostsResponse>) {
    ...
    try {
      ...
      const total = restrictedPosts
        ? await this.postManager.getPostsByUserTotal(userId)
        : await this.feedManager.getSiteTotal(siteId)
      const rawPosts =
        restrictedPosts || (await this.feedManager.getSiteFeed(userId, siteId, page, perPage, format, sorting))
  ...
  async feedWatch(request: APIRequest<FeedWatchRequest>, response: APIResponse<FeedWatchResponse>) {
    ...
    try {
      ...
      const total = restrictedPosts
        ? await this.postManager.getPostsByUserTotal(userId)
        : await this.feedManager.getWatchTotal(userId, filter === 'all')
      const rawPosts =
        restrictedPosts || (await this.feedManager.getWatchFeed(userId, page, perPage, filter === 'all', format))
      const { posts, users, sites } = await this.enricher.enrichRawPosts(rawPosts)
