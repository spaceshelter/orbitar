...
export default function extractFromExpressApp<T>(
    ...
    if (Array.isArray(app)) {
        app.forEach(layer => {
            const layerResults = extractFromExpressApp<T>(
                layer,
                propertyName,
    ...
    if (layer.handle && typeof layer.handle === 'function') {
        ...
        const scopeProperties = Object.getOwnPropertyNames(handle)
            .reduce<Record<string, unknown>>((obj, key) => {
                obj[key] = (handle as unknown as Record<string, unknown>)[key];
                return obj;
            }, {});

        const handleResults = extractFromExpressApp<T>(
            scopeProperties,
            propertyName,
            currentPath
        );
    ...
    if (layer.route?.stack) {
        const routeResults = extractFromExpressApp<T>(
            layer.route.stack,
            propertyName,
            layer.route.path
        );
    ...
    if (layer.stack) {
        const stackResults = extractFromExpressApp<T>(
            layer.stack,
            propertyName,
            currentPath
        );
    ...
    // Check params object
    if (layer.params) {
        const paramsResults = extractFromExpressApp<T>(
            layer.params,
            propertyName,
            currentPath
        );
