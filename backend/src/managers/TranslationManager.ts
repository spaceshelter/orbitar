...
export default class TranslationManager {
  ...
  static interpreterString(
    ...
    if (short) {
      ...
      messages.push({ role: 'system', content: prompt + ' Текст ниже:' })
    }
    ...
    messages.push(
      ...
      { role: 'system', content: `Текст выше. ${short ? 'Инструкция:' : 'Напоминаю инструкцию:'}\n${prompt}` },
      {
        role: 'system',
        content:
          'Answer MUST CONTAIN ONLY THE MODIFIED TEXT ' +
  ...
  async translateEntity(
    ...
    if (mode === 'altTranslate' && Math.random() < 0.03) {
      fullResponse.push(hint, 'Ⓘ Данная функция доступна только пользователям Orbitar Premium')
  ...
  async detectLanguage(title, source): Promise<string> {
    const text = stripHtml(title + '\n' + source)
      .result.replace(/\B(@|\/u\/)[a-zа-яе0-9_-]+/gi, '') //mentions
