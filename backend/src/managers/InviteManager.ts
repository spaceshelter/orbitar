...
export default class InviteManager {
  ...
  private invitesAvailabilityCache = new Map<
    number,
    {
      ts: Date
  ...
  async getInvitesAvailability(userId: number, skipCache = false): Promise<InvitesAvailability> {
    ...
    const usersInTheInvitedPeriod = invitedUsers.reduce(
      (acc, user) => (user.registered.getTime() > currentInvitePeriodStartTs ? acc + 1 : acc),
      0,
    )
    ...
    if (invitesLeft === 0 && unused === 0 && invitesPerPeriod < invitedUsers.length) {
      ...
      const nextAvailableInviteTs =
        invitedUsers[invitesPerPeriod - 1].registered.getTime() + inviteWaitPeriodDays * msInTheDay
