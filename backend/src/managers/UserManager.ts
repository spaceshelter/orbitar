...
export default class UserManager {
    ...
    async getByUsernameWithVoteAndBio(username: string, forUserId: number): Promise<UserInfo | undefined> {
        const infoWithoutVoteAndBio = await this.getByUsername(username, true);
        if (!infoWithoutVoteAndBio) {
            return;
        }
        const vote = await this.voteRepository.getUserVote(infoWithoutVoteAndBio.id, forUserId);
        return {vote, ...infoWithoutVoteAndBio};
    }
