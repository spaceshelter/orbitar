...
export default class UserManager {
  ...
  private userRestrictionsCache = new Map<
    number,
    {
      ts: Date
  ...
  async getUserStats(forUserId: number): Promise<UserStats> {
    ...
    const unreadComments = await this.userRepository.getUserUnreadComments(
      forUserId,
  ...
  async getNumActiveUsersThatCanVote() {
    if (
      this.cachedNumActiveUsersThatCanVote.lastUpdateTs + this.cachedNumActiveUsersThatCanVote.expirationMs >
      Date.now()
    ) {
  ...
  async getUserEffectiveKarma(userId: number): Promise<{
    ...
    const contentVal = (userContentRating * (userContentRating >= 0 ? 1 : NEGATIVE_CONTENT_MULTIPLIER)) / CONTENT_RATING_NORMALIZER
    ...
    const userRating =
      profileVotingResult >= 0 ? 1 : Math.max(0, 1 - lerp(Math.pow(ratio / REPUTATION_RATIO_NORMALIZER, 2), Math.pow(ratio * REPUTATION_RATIO_AMPLIFIER, 2), s))
  ...
  async getUserRestrictions(userId: number): Promise<UserRestrictions> {
    ...
    const lastOwnPost = localCachedValue
      ? localCachedValue.lastOwnPost
    ...
    const commentSlowModeDelay =
      effectiveKarma <= USER_RESTRICTIONS.MIN_KARMA
        ? /*1 hour */ 3600
        : effectiveKarma < NEG_KARMA_THRESH
          ? /*10 minutes */ 10 * 60
          : 0
  ...
  async getTrialVoters(userId: number, threshold): Promise<number[]> {
    ...
    for (const { userId, voterId, vote } of secondaryVoters) {
      ...
      secondaryVotesIdx.set(voterId, (secondaryVotesIdx.get(voterId) || 0) + Math.sign(vote) * Math.sign(primaryVote))
    }
  ...
  async getTrialProgressRaw(userId: number): Promise<TrialProgressDebugInfo> {
    ...
    if (user?.ontrial) {
      ...
      const singleVotesPart = Math.min(
        1,
        (secondaryVoters[1] + secondaryVoters[2]) / (activeUsersThatCanVoteNum * SINGLE_VOTE_THRESHOLD),
      )
  ...
  async getTrialProgress(userId: number, skipCache = false) {
    ...
    const { effectiveKarmaPart, singleVotesPart, doubleVotesPart, daysOnSitePart } =
      await this.getTrialProgressRaw(userId)
    const sharedPart = effectiveKarmaPart * 0.5 + daysOnSitePart * 0.5

    const progress =
      singleVotesPart === undefined || doubleVotesPart === undefined
        ? sharedPart
  ...
  async getUserVisitedDaysAgo(userId: number): Promise<number | null> {
    ...
    return lastVisited ? Math.floor(Math.abs(new Date().getTime() - lastVisited.getTime()) / (3600 * 24 * 1000)) : null
  }
}
