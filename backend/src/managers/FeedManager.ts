...
export default class FeedManager {
    ...
    if (!this.initialized || force) {
      ...
      this.initialized = (async () => {
        ...
        try {
          ...
          await fetch(`${FEED_API}/clear`, { method: 'POST' })
          ...
          while (true) {
            ...
            for (const post of batch) {
              addPostToBatch(`${post.site_id}:new`, this.offsetPostTs(post.created_at), post.post_id)
              addPostToBatch(`${post.site_id}:live`, this.offsetPostTs(post.commented_at), post.post_id)
  ...
  async getSubscriptionFeed(
    ...
    const {
      total,
      post_ids: postIds,
      cache_is_empty: cacheIsEmpty,
    } = await fetch(`${FEED_API}/query`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    const posts: PostRawWithUserData[] = await this.postRepository.getPostsWithUserData(postIds, forUserId)

    // sort by the order of ids in the query
    posts.sort((a, b) => postIds.indexOf(a.post_id) - postIds.indexOf(b.post_id))
  ...
  async convertRawPosts(
    forUserId: number,
    rawPosts: PostRawWithUserData[],
    format: ContentFormat,
  ): Promise<PostInfo[]> {
    ...
    this.postRepository
      .updateHtmlAndParserVersion(
        commentsToUpdateHtmlAndParserVersion.filter((x) => x.html !== undefined),
        TheParser.VERSION,
      )
      .then()
      .catch()
  ...
  async siteSubscribe(userId: number, siteName: string, main: boolean, bookmarks: boolean) {
    ...
    if (
      ...
      !!existingSubscription.feed_main === main &&
