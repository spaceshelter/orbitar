...
export default class UserCredentials {
  ...
  async setCredential<T>(forUserId: number, type: string, value: T) {
    ...
    await this.db.query(
      'insert into user_credentials (`user_id`, `type`, `value`) values(:user_id, :type, :value) on duplicate key update value=:value',
      {
        user_id: forUserId,
        type,
        value: stringValue,
  ...
  async getCredential<T>(forUserId: number, type: string): Promise<T | undefined> {
    const result = await this.db.fetchOne<{ value: string }>(
      'select value from user_credentials where user_id=:user_id and `type` = :type',
      {
        user_id: forUserId,
        type,
      },
