...
export default class UserRepository {
  ...
  async getPasswordHashByUserId(userId: number): Promise<string | undefined> {
    const result = await this.db.fetchOne<{ password: string }>('select password from users where user_id=:user_id', {
      user_id: userId,
    })
  ...
  async getResetPasswordUserIdByResetCode(code: string) {
    return await this.db.fetchOne<{ user_id: number }>(
      'select user_id from user_password_reset where code=:code and generated_at > now() - interval ' +
        this.passwordResetCodeLifeTime,
      {
        code,
      },
    )
  ...
  async clearResetPasswordExpiredLinks() {
    await this.db.query(
      `delete from user_password_reset where generated_at < now() - interval ` + this.passwordResetCodeLifeTime,
    )
    return true
  }
  ...
  async getUserParent(userId: number): Promise<UserRaw | undefined> {
    return await this.db.fetchOne<UserRaw>(
      'select u.* from user_invites i join users u on (i.parent_id = u.user_id) where i.child_id=:user_id',
      {
        user_id: userId,
      },
    )
  ...
  async getUserChildren(userId: number): Promise<UserRaw[]> {
    return await this.db.query<UserRaw[]>(
      'select u.* from user_invites i join users u on (i.child_id = u.user_id) where i.parent_id=:user_id order by i.invited',
      {
        user_id: userId,
      },
    )
  ...
  async userRegister(
    ...
    return await this.db.inTransaction(async (conn) => {
      const inviteRow: InviteRaw = await conn.fetchOne(
        'select i.* from invites i where code=:code and left_count > 0',
        { code: inviteCode },
      )
      if (!inviteRow) {
      ...
      const userCountResult = await conn.query<{ cnt: string }>(
        'select count(*) cnt from users where username=:username',
        { username: username },
      )
      ...
      await conn.query(
        'insert into user_invites (parent_id, child_id, invited, invite_id) values(:parent_id, :child_id, now(), :invite_id)',
        {
      ...
      const userRow = await conn.fetchOne<UserRaw>('select * from users where user_id=:user_id', {
        user_id: userInsertId,
      })
      if (!userRow) {
  ...
  async getUserMainSubscriptions(userId: number): Promise<UserSiteRaw[]> {
    return await this.db.fetchAll<UserSiteRaw>(`select * from user_sites where user_id=:user_id and feed_main=1`, {
      user_id: userId,
    })
  }

  async getMainSubscriptionsUsers(siteId: number): Promise<{ user_id: number }[]> {
    return await this.db.fetchAll<{ user_id: number }>(
      'select user_id from user_sites where site_id=:site_id and feed_main=1',
      { site_id: siteId },
    )
  }
  ...
  logVisit(userId: number, date: Date): Promise<void> {
    ...
    const dateToInsert = date.toISOString().replace('T', ' ').substring(0, 19) + '.' + date.getMilliseconds()
  ...
  async saveFeedSorting(site: string, feedSorting: FeedSorting, userId: number) {
    await this.db.query(
      `insert into feed_sorting_settings (user_id, site_id, feed_sorting) select :user_id, s.site_id, :feed_sorting from sites s where s.subdomain = :site on duplicate key update feed_sorting = :feed_sorting`,
      {
        user_id: userId,
        site: site,
  ...
  async getFeedSorting(userId: number, siteId: number): Promise<FeedSorting> {
    const res = await this.db.fetchOne<{ feed_sorting: FeedSorting } | undefined>(
      'select fss.feed_sorting from feed_sorting_settings fss where fss.user_id=:user_id and fss.site_id=:site_id',
