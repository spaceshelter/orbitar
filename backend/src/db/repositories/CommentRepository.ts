...
export default class CommentRepository {
  ...
  async getLastUserComment(userId: number): Promise<CommentRaw | undefined> {
    return await this.db.fetchOne<CommentRaw>(
      'select * from comments where author_id=:user_id order by created_at desc limit 1',
      {
        user_id: userId,
      },
    )
  ...
  async createComment(
    ...
    return await this.db.inTransaction(async (conn) => {
      const siteResult = await conn.fetchOne<{ site_id: number }>('select site_id from posts where post_id=:post_id', {
        post_id: postId,
      })
  ...
  getUserIdByCommentId(commentId: number): Promise<number | undefined> {
    return this.db
      .fetchOne<{ author_id: number }>(`select author_id from comments where comment_id = :commentId`, {
        commentId,
      })
