...
export default class WebPushRepository {
  ...
  async setSubscription(forUserId: number, subscription: PushSubscription) {
    ...
    await this.db.query(
      'insert into user_webpush (`user_id`, `auth`, `subscription`) values(:user_id, :auth, :subscription) on duplicate key update subscription=:subscription',
      {
        user_id: forUserId,
  ...
  async getSubscription(forUserId: number, auth: string): Promise<PushSubscription | undefined> {
    const result = await this.db.fetchOne<{ subscription: string }>(
      'select subscription from user_webpush where user_id=:user_id and auth = :auth',
      {
        user_id: forUserId,
        auth,
      },
  ...
  async getSubscriptions(forUserId: number): Promise<PushSubscription[]> {
    const results = await this.db.fetchAll<{ subscription: string }>(
      'select subscription from user_webpush where user_id=:user_id',
      {
        user_id: forUserId,
      },
    )
