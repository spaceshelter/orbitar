...
export default class SiteRepository {
  ...
  async createSite(forUserId: number, site: string, name: string): Promise<SiteWithUserInfoRaw> {
    return await this.db.inTransaction(async (conn) => {
      ...
      const siteExistsResult = await conn.fetchOne<{ site_id: number }>(
        `select site_id from sites where subdomain=:site`,
        { site },
      )
      if (siteExistsResult) {
      ...
      const siteCountResult = await conn.fetchOne<{ cnt: string }>(
        `select count(*) cnt from sites where owner_id=:forUserId`,
        { forUserId },
      )
  ...
  async subscribe(userId: number, siteId: number, main: boolean, bookmarks: boolean) {
    return await this.db.inTransaction(async (conn) => {
      const result = await conn.query(
        'insert into user_sites (site_id, user_id, feed_main, feed_bookmarks) values (:site_id, :user_id, :main, :bookmarks) on duplicate key update feed_main=:main, feed_bookmarks=:bookmarks',
        {
          site_id: siteId,
          user_id: userId,
          main: main ? 1 : 0,
      ...
      await conn.query(
        'update sites s set s.subscribers=(select count(*) from user_sites us where us.site_id=:siteId and us.feed_main=1) where s.site_id=:siteId',
        { siteId },
      )
