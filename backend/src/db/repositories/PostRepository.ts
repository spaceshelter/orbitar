...
export default class PostRepository {
  ...
  getLastUserPost(userId: number): Promise<PostRaw | undefined> {
    return this.db.fetchOne<PostRawWithUserData>(
      'select * from posts where author_id=:user_id order by created_at desc limit 1',
      { user_id: userId },
    )
  }
  ...
  async getSitePostUpdateDates(
    forUserId: number,
    siteId: number,
    afterPostId: number,
    limit: number,
  ): Promise<{ post_id: number; commented_at: Date; created_at: Date }[]> {
  ...
  async getContentSources(refId: number, refType: string): Promise<ContentSourceRaw[]> {
    return await this.db.fetchAll<ContentSourceRaw>(
      'select * from content_source where ref_id=:refId and ref_type=:refType order by content_source_id desc',
      {
        refId,
        refType,
      },
  ...
  getUserIdOverride(postId: number): Promise<number | undefined> {
    return this.db
      .fetchOne<{ user_id: number }>('select user_id from anonymous_posts where post_id=:postId', {
        postId,
      })
  ...
  async getUserIdByPostId(postId: number) {
    return await this.db
      .fetchOne<{ author_id: number }>('select author_id from posts where post_id=:postId', {
        postId,
      })
